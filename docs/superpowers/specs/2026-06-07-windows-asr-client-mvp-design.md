...
> [!IMPORTANT]
> 本文是 MVP 阶段的历史规格，保留当时决策。听写模式名称、右 Alt / 右 Ctrl / F9 职责、连续分段、浮窗与取消行为已由 [就地听写与独立听写设计](./2026-08-18-in-place-and-independent-dictation-design.md) 取代；其他 MVP 范围仍可作为历史背景参考。
...
当用户缩短保留期限时，客户端应在保存前计算超出新期限的文本历史数量；存在待删除记录时显示不可恢复的删除确认，用户确认后立即清理，取消时不保存本次设置。文本历史清理不影响使用统计。
