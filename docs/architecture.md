...
1. 用户按下右 Alt；App 在首次 `KeyDown` 时捕获当前前台窗口、可编辑控件、光标位置及前台窗口所在显示器，取得活动租约，在该显示器工作区底部居中显示不激活的紧凑波形浮窗并开始录音。显示器在本次会话内保持固定；无法取得目标窗口时依次回退到鼠标所在显示器和主显示器。右 Alt 的按下、自动重复与松开事件均由 LessASR 拦截，不传给前台应用。
...
`CachedStatsRepository` 在启动时一次性加载数据库中保留的每日统计，供所有听写路径共享。每次写入或清理先完成数据库操作，再同步更新按日期和模型配置组织的内存统计；读写串行执行，写入失败不改内存。范围查询只返回不可变记录的内存快照，不再次查询数据库，也不向界面发布统计变更事件。首页与统计页共用的 `StatsViewModel` 仅在初始化、主窗口从隐藏恢复显示、切回首页或统计页时主动读取内存快照，并按本地当天日期重新计算汇总和趋势；页面持续打开或仅重新获得焦点时不刷新。关闭历史保存或独立听写窗口尚未关闭不影响内存统计更新。
...
- `AsrRequest` 分别携带词汇提示 `InitialPrompt` 与按首选语言生成的 `LanguageStylePrompt`。`TranscriptionPromptComposer` 为简体、繁体、英文提供带句末标点的示例；自动与其他语言暂不附加。后端在调用 HTTP 客户端前组合两者，远程 `UseVocabulary` 仅过滤词汇提示。就地、独立、旧编排入口和远程配置测试使用一致规则，示例不写入词汇表或识别结果。
