...
> [!IMPORTANT]
> 本文记录旧版“右 Ctrl 单句听写”流程，保留为历史规格。现行的“就地听写 / 独立听写”、右 Alt / 右 Ctrl / F9 职责、连续分段、两阶段 Esc 与展开浮窗以 [2026-08-18 设计](./2026-08-18-in-place-and-independent-dictation-design.md) 为准。
...
- **触发方式**：完成一次**按下并释放**后，在释放瞬间触发一次；忽略按住期间的重复事件。
- **单键判定**：右 Ctrl 按下前已有其他键处于按下状态，或右 Ctrl 按下后、释放前又按下任何其他键，本次手势均作废，不触发听写。
- **组合键兼容**：右 Ctrl 的原始按下与释放消息继续传递给前台应用，使其仍可作为 `Ctrl+C`、`Ctrl+V` 等组合键的修饰键。
