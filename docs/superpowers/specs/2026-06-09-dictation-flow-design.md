...
识别完成后，优先将文本写入**当前前台线程焦点控件**，且默认不使用剪贴板。

### 实现策略

`SendInputTextInjector` 为 MVP 默认实现，分两步：

1. **焦点检测（待实现）**：判断当前焦点是否可编辑。推荐先用 Win32 `GetGUIThreadInfo` / `GetFocus` 结合控件类型探测；若不足再评估 UI Automation。
2. **注入**：对可编辑目标，通过 `SendInput` 发送 `KEYEVENTF_UNICODE` 逐字符输入。
