...
- `token_timestamps`：固定为 `false`；听写不需要词元时间戳，显式关闭以绕过 whisper.cpp 部分版本默认固定长度折行的问题（[上游 #3968](https://github.com/ggml-org/whisper.cpp/issues/3968)）
- `language`：可选；首选语言非自动时发送 Whisper 语言代码，例如 `zh`、`en`
- `prompt`：可选；组合当前词汇表与首选语言的行文示例，规则见下文“提示文本”
...
- 关闭词元时间戳只绕过上述额外折行，不删除服务返回的自然分段换行，也不补标点或用空格合并分段；右 Ctrl 手动分段的合并规则保持不变。此参数仅发往本地 whisper-server。
...
- `prompt`：可选；组合该配置允许使用的当前词汇表与首选语言的行文示例。关闭词汇表只排除用户词条，仍可发送语言示例
...
每份远程配置可以独立填写代理服务器 URL；留空时沿用 Windows/.NET 的系统代理配置。支持：
...
代理一旦配置，对手动测试和真实转写同时生效，也不会因为目标位于本机或局域网而绕过。代理 URL 只能包含协议、主机和可选端口，不能包含路径、查询参数、片段或账号密码；当前版本不支持认证代理。

客户端按规范化后的代理地址复用 `HttpClient` 与连接池，不为每次转写重新创建连接。
...
- 使用与真实转写相同的完整端点、模型、认证方式、代理和当前首选语言；
- 该配置启用词汇表时查询当前使用中的词汇表，关闭时不查询词汇表；两种情况均按当前首选语言组合行文示例，与真实听写一致；
- 不切换当前服务；
- 可能被提供方计费；
- 不触发自动重试。
...
本地与远程服务共用 `TranscriptionPromptComposer`。每段识别与远程配置测试都从本次读取的首选语言生成行文示例：

| 首选语言 | 自动附加的示例 |
| --- | --- |
| 简体中文（`zh-Hans`） | `以下是普通话的句子。` |
| 繁体中文（`zh-Hant`） | `以下是普通話的句子。` |
| English（`en`） | `This is a sentence in English.` |
| 自动、其他语言或未知值 | 暂不附加示例 |

词汇表仍按既有规则逆序以英文逗号连接，保持列表顶部词条更靠近提示末尾的相对优先级；行文示例放在词汇文本之后，用一个换行分隔。没有词汇时只发送示例；两者均为空时省略 `prompt`。如果整个词汇提示恰好等于示例，只发送一次。

`AsrRequest.InitialPrompt` 携带词汇提示，`LanguageStylePrompt` 独立携带行文示例；本地后端直接组合两者，远程后端先按 `UseVocabulary` 过滤词汇提示再组合。因此简繁偏好即使都映射为 HTTP `language=zh`，也保留不同的提示文本。

示例在请求时生成，不写入用户词汇表，不占用词条名额；语言偏好保存后从下一次请求生效。它是提供给模型的文字风格示例，不保证标点完整，也不会直接拼入识别正文。服务返回的文本继续经过既有后处理，不额外合并换行或猜测逗号、句号。
