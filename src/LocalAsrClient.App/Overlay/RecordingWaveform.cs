...
{
    internal const int InputLevelsPerBar = 2;
    private const double PreferredBarSpacing = 8d / 3;
    private const double DisplayGain = 1.5;
    ...
    public void PushLevel(float level, bool animate = true)
    {
        ...
        // Combine two 50 ms audio updates into one bar without losing brief peaks.
        // The narrower spacing preserves the previous horizontal scrolling speed.
    ...
    protected override void OnRender(DrawingContext drawingContext)
    {
        ...
        var maximumHalfHeight = Math.Max(0, (ActualHeight - 2) / 2);
        var barPen = CreatePen(WaveformBrush, 1);
        ...
        for (var visibleIndex = firstDrawnIndex; visibleIndex < visibleBarCount; visibleIndex++)
        {
            ...
            if (level <= 0)
            {
                // Silence uses the same position and width as a collapsed waveform bar.
                drawingContext.DrawEllipse(WaveformBrush, null, new Point(x, centerY), 0.5, 0.5);
                continue;
            }

            // Boost only the visualization; recorded audio keeps its original level.
            var displayLevel = Math.Min(1, level * DisplayGain);
            var halfHeight = Math.Pow(displayLevel, 0.72) * maximumHalfHeight;
