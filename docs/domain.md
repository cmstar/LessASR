...
| RemoteApiProfile | 一份命名的远程 API 配置：完整端点、模型、DPAPI 密文或空 Key、是否启用词汇表 |
...
- 远程配置名称必须唯一；端点是完整的 Audio Transcriptions URL；模型不能为空；API Key 可为空。已有配置的 Key 输入框留空表示保留，只有显式“清除已保存 Key”才删除密文。
